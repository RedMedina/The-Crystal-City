...
public:
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, D3DXVECTOR3 poscam, float xx, float zz, float posy, float escala, vector2* uv1, vector2* uv2, vector2* uv3, vector2* uv4, int frame, bool Anim, bool EscalarenY)
	{
		posx = xx;
		posz = zz;
		vertices = new VertexComponent[4];
		...
		{
			vertices[1].pos.x = 0;
			vertices[1].pos.y = 2 * escala * 1.5f;
			vertices[1].pos.z = -1 * escala;
			vertices[1].UV.x = uv2[frame].u;
			vertices[1].UV.y = uv2[frame].v;

			vertices[2].pos.x = 0;
			vertices[2].pos.y = 2 * escala * 1.5f;
			vertices[2].pos.z = 1 * escala;
