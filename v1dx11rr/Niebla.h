...
public:
	Niebla(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* specularTexturePath, WCHAR* normalTexturePath, float _posX, float _posZ)
	{
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex, WCHAR* normalTex)
	{
		...
		bool compileResult = CompileD3DShader(L"Niebla.fx", "VS_Main", "vs_4_0", &vsBuffer);
		...
		compileResult = CompileD3DShader(L"Niebla.fx", "PS_Main", "ps_4_0", &psBuffer);
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &color);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, bool MovCam, int tipoCam, float PositionX, float PositionZ, int TipoModelo, float colorc)
	{
		/*
			1.- Pasto
			2.- Bola pinchos
		*/
		...
		d3dContext->UpdateSubresource(color, 0, 0, &colorc, 0, 0);
		...
		d3dContext->PSSetConstantBuffers(5, 1, &color);
