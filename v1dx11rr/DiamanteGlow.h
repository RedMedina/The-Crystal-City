...
public:
	DiamanteGlow(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* specularTexturePath, WCHAR* normalTexturePath, WCHAR* glowTexture, float _posX, float _posZ)
	{
		...
		CargaParametros(ModelPath, colorTexturePath, specularTexturePath, normalTexturePath, glowTexture);//L"Assets/Tent-Tower/tent_diffuse.jpg"
	}
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex, WCHAR* normalTex, WCHAR* glowTex)
	{
		HRESULT d3dResult;
		...
		bool compileResult = CompileD3DShader(L"Glow.fx", "VS_Main", "vs_4_0", &vsBuffer);
		...
		compileResult = CompileD3DShader(L"Glow.fx", "PS_Main", "ps_4_0", &psBuffer);
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, glowTex, 0, 0, &GLOWMap, 0);
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &Red);
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &Blue);
		//de vista
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, bool MovCam, int tipoCam, float PositionX, float PositionZ, int TipoModelo, float RedC, float BlueC)
	{
		/*
			1.- Pasto
			2.- Bola pinchos
		*/
		...
		d3dContext->PSSetShaderResources(3, 1, &GLOWMap);
		...
		D3DXMATRIX scaleMat;
		D3DXMatrixScaling(&scaleMat, scale, scale * 1.5f, scale);
		...
		d3dContext->UpdateSubresource(Red, 0, 0, &RedC, 0, 0);
		d3dContext->UpdateSubresource(Blue, 0, 0, &BlueC, 0, 0);
		...
		d3dContext->PSSetConstantBuffers(5, 1, &Red);
		d3dContext->PSSetConstantBuffers(6, 1, &Blue);
