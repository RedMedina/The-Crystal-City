...
private:
	...
	ID3D11ShaderResourceView* texturaStar;
	ID3D11ShaderResourceView* texturaBlend;
...
public:
	SkyDome(int slices, int stacks, float radio, ID3D11Device** d3dDevice,
		ID3D11DeviceContext** d3dContext, WCHAR* diffuseTex, WCHAR* Star, WCHAR* Blend)
	{
		this->slices = slices;
		this->stacks = stacks;
		this->radio = radio;
		...
		LoadContent(diffuseTex, Star, Blend);
	}
	...
	bool LoadContent(WCHAR* diffuseTex, WCHAR* Star, WCHAR* Blend)
	{
		HRESULT d3dResult;
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), Star, 0, 0, &texturaStar, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), Blend, 0, 0, &texturaBlend, 0);
		...
		d3dResult = (*d3dDevice)->CreateBuffer(&constDesc, 0, &sky1);
		d3dResult = (*d3dDevice)->CreateBuffer(&constDesc, 0, &sky2);
	...
	void Render(D3DXVECTOR3 trans, float sky, float red)
	{
		if (d3dContext == 0)
			return;
		...
		(*d3dContext)->PSSetShaderResources(1, 1, &texturaStar);
		(*d3dContext)->PSSetShaderResources(2, 1, &texturaBlend);
		(*d3dContext)->PSSetSamplers(0, 1, &texSampler);
		//(*d3dContext)->UpdateSubresource(Colores,0,0,&COLORS,0,0);
		//(*d3dContext)->PSSetConstantBuffers(1, 1, &Colores);
		...
		i < 360 ? i += 0.0005f : i = 0;
		D3DXMatrixRotationY(&rotationMat, i);
		D3DXMatrixTranspose(&worldMat, &worldMat);
		matrices->worldMatrix = worldMat * rotationMat;
		//matrices->worldMatrix = worldMat;

		(*d3dContext)->UpdateSubresource(matrixBufferCB, 0, 0, matrices, sizeof(MatrixType), 0);
		(*d3dContext)->UpdateSubresource(sky1, 0, 0, &sky, 0, 0);
		(*d3dContext)->UpdateSubresource(sky2, 0, 0, &red, 0, 0);
		(*d3dContext)->VSSetConstantBuffers(0, 1, &matrixBufferCB);

		(*d3dContext)->PSSetConstantBuffers(2, 1, &sky1);
		(*d3dContext)->PSSetConstantBuffers(3, 1, &sky2);
