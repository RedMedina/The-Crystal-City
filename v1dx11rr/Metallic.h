...
public:
	Metallic(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* specularTexturePath, WCHAR* normalTexturePath, WCHAR* metallicTexturePath, float _posX, float _posZ)
	{
		...
		CargaParametros(ModelPath, colorTexturePath, specularTexturePath, normalTexturePath, metallicTexturePath);//L"Assets/Tent-Tower/tent_diffuse.jpg"
	}
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex, WCHAR* normalTex, WCHAR* metallicTex)
	{
		HRESULT d3dResult;
		...
		bool compileResult = CompileD3DShader(L"Metallic.fx", "VS_Main", "vs_4_0", &vsBuffer);
		...
		compileResult = CompileD3DShader(L"Metallic.fx", "PS_Main", "ps_4_0", &psBuffer);
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, metallicTex, 0, 0, &metallicMap, 0);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, bool MovCam, int tipoCam, float PositionX, float PositionZ, int TipoModelo)
	{
		/*
			1.- Pasto
			2.- Bola pinchos
		*/
		...
		d3dContext->PSSetShaderResources(3, 1, &metallicMap);
