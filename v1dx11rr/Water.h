...
private:
	...
	ID3D11ShaderResourceView* cubeMapp;
	...
	ID3D11Buffer* ReflectMatrix;
	float RMatrix;

	ID3D11Buffer* ReflectionRefractScale;
	float RRScale;

	ID3D11Buffer* WaterTranslation;
	float WaterT;
...
public:
	WateRR(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* specularTexturePath, WCHAR* normalTexturePath, WCHAR* cubeTextureMap, float _posX, float _posZ)
	{
		...
		CargaParametros(ModelPath, colorTexturePath, specularTexturePath, normalTexturePath, cubeTextureMap);//L"Assets/Tent-Tower/tent_diffuse.jpg"
	}
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* specularTex, WCHAR* normalTex, WCHAR* cubeMap)
	{
		HRESULT d3dResult;
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, cubeMap, 0, 0, &cubeMapp, 0);
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &ReflectMatrix);
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &ReflectionRefractScale);
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &WaterTranslation);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, bool MovCam, int tipoCam, float PositionX, float PositionZ, int TipoModelo, D3DXMATRIX ReflejoMatrix)
	{
		/*
			1.- Pasto
			2.- Bola pinchos
		*/
		...
		d3dContext->PSSetShaderResources(3, 1, &cubeMapp);

		d3dContext->PSSetSamplers(0, 1, &colorMapSampler);

		//mueve la camara
		D3DXMATRIX RotacionCam;

		if (tipoCam == 1)
		{
			D3DXMatrixTranslation(&RotacionCam, 0, 0, -10);
		}
		else if (tipoCam == 3)
		{
			//D3DXMatrixTranslation(&RotacionCam, 0, 0, 0);
		}
		else
		{
			D3DXMatrixTranslation(&RotacionCam, 0, 0, -20);
		}
		...
		D3DXMATRIX scaleMat;
		D3DXMatrixScaling(&scaleMat, scale * 3.0f, scale * 1.5f, scale * 1.9f);

		D3DXMATRIX worldMat;

		if (MovCam)
		{
			worldMat = scaleMat * RotacionCam * rotationMat * translationMat;
		}
		else
		{
			switch (TipoModelo)
			...
			case 1:
				i < 360 ? i += 1.0f : i = 0;
				//D3DXMatrixRotationX(&rotationMat, i);
				D3DXMatrixTranslation(&translationMat, PositionX, ypos, PositionZ);
		...
		d3dContext->UpdateSubresource(ReflectMatrix, 0, 0, &ReflejoMatrix, 0, 0);
		d3dContext->UpdateSubresource(ReflectionRefractScale, 0, 0, &PositionX, 0, 0);
		d3dContext->UpdateSubresource(WaterTranslation, 0, 0, &PositionX, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(5, 1, &ReflectMatrix);
		d3dContext->VSSetConstantBuffers(6, 1, &ReflectionRefractScale);
		d3dContext->VSSetConstantBuffers(7, 1, &WaterTranslation);
