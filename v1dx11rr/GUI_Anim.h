...
public:
	GUI_Anim(ID3D11Device* d3d11Device, ID3D11DeviceContext* d3dContext, float Height, float Width, LPCWSTR texturePath) {
		this->Height = Height;
		this->Width = Width;
	...
	void createBitMap() {
		...
		d3d11Device->CreateTexture2D(&textureDesc, NULL, &TexturaUI);
	...
	void Draw(float posX, float posY, float ColorChange) {
		...
		D3DXMatrixTranslation(&Traslation, posX, posY, 0.8);
		D3DXMATRIX World = Scale * Traslation;
		D3DXMatrixTranspose(&consBuff.WorldMatrix, &World);
		d3dContext->UpdateSubresource(cbBuffer, 0, NULL, &consBuff, 0, 0);
		d3dContext->UpdateSubresource(Color, 0, 0, &ColorChange, 0, 0);
		d3dContext->VSGetConstantBuffers(0, 1, &cbBuffer);
		d3dContext->PSSetConstantBuffers(1, 1, &Color);
